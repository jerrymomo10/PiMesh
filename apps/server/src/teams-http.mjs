import { createAuth, createLimiter, failure } from './auth.mjs';
import { basicMatches, json, token } from './auth-http.mjs';
import { createTeams } from './teams.mjs';

export function createTeamsHttp(pool, { origin, adminHash, limiter = createLimiter() }) {
  const teams = createTeams(pool), auth = createAuth(pool);
  return async (req, res, url) => {
    ...
    try {
      ...
      } else { user = (await auth.me(token(req))).user_id; }
      ...
      if (path === '/api/v1/admin/teams') {
        ...
        if (target) {
          ...
          send(post ? 201 : 200, post ? { project: await teams.createProject(user, id, data) } : await teams.projects(user, id, page));
        } else {
