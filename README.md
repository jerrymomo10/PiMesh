...
提供账号登录、团队邀请和项目目录。研究记录协作与共享 Memory 继续建设中。
</td>
</tr>
</table>

> **当前阶段：早期开发。** 本地客户端和团队目录只读服务已实现；服务端 0.3.0 提供平台注册、管理员创建团队、团队邀请和项目创建，需完成迁移并启用账号配置。跨端同步尚未实现。当前不会自动上传 Transcript。
...
这里的 `/login` 是模型认证，不是团队登录。团队登录使用 `meshpi team login`，完整流程见 [同学使用指南](docs/team-client.md)。模型请求使用所选供应商的认证与计费。
...
临时查看账号不是团队身份体系。账号与管理员邀请码入口见 [注册登录指南](docs/auth.md)，团队工作台与权限见 [团队指南](docs/teams.md)。按 [服务部署指南](docs/team-service.md) 配置独立实例。

注册密码长度为 8–128 位。普通用户从 `/` 工作台进入 `/account` 登录；管理员从 `/admin/teams` 管理团队、`/admin/invites` 管理注册邀请。全站目录位于 `/admin/directory`，仍使用独立查看账号。
...
| 团队身份 | 管理员创建团队、负责人邀请/移除成员、团队隔离 | 服务端 0.3.0；设备注册与 CLI 接入见使用指南 |
| 项目协作 | 负责人创建项目、成员查看团队项目 | 服务端 0.3.0；CLI 支持本地绑定，研究记录共享待实现 |
| 共享知识 | 实验记录、Memory 检索、工具沉淀与周报 | 设计中 |

个人完整 Transcript 不对其他成员开放；Memory 分层、上传备份与同步策略尚未定案。路线图是方向说明，不是已经交付的功能承诺。
