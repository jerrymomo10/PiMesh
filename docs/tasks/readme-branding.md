...
- 首版 GitHub 渲染验证内容提交：5f73aca。
- 本次调整：移除副标题，文案突出多位研究员、多端与 Agent 过程中的自然协作；仅重画架构图，其他首页内容保持不变。
- 本次在 macOS 使用 Quick Look 检查完整 SVG 渲染，XML 解析与 git diff --check 通过；验证版本为本条记录所在提交。
